...
export async function POST(request: NextRequest) {
    try {
        ...
        const sigAuthRedirect = request.headers.get("x-sigauth-redirect");
        ...
        if (!isValidRequestData(placeId, items, description, total, account, type)) {
            return NextResponse.json(
                { error: "Invalid request data" },
                { status: 400 }
            );
        ...
        const { data: availableItems, error } = await getItemsForPlace(client, placeId);
        if (error) {
            return NextResponse.json(
                { error: error.message },
                { status: 500 }
        ...
        const invalidItem = items.find((item: { id: number; }) => !availableItemIds.has(item.id));
        if (invalidItem) {
            return NextResponse.json(
 ...
 */
function isValidItemsFormat(items: any[]): boolean {
    return items.every(
        (item) =>
            typeof item === "object" &&
